...
app.get('/gestionTiemposComida',async (req, res)=>{
	connection.query('SELECT * FROM comida', async(error, results)=>{
		if(error){
			console.log(error);
		}else{
...
app.get('/gestionPedidos',async (req, res)=>{
	connection.query('SELECT * FROM pedidos', (error, results)=>{
		if(error){
			console.log(error);
		}else{
			res.render('gestionPedidos',{
				results: results,
				nombre: nombreU,
				correo: correoU
			});
...
//GESTION TIEMPOS DE COMIDA
app.post('/seleccionGestionTiemposComida', async(req, res)=>{
	console.log("entre");
	var tiempo = req.body.seleccionTiempo;
	var tipo = req.body.seleccionTipo;
	if(tiempo == 0){
		tiempo = "Desayuno";
	}
	if(tiempo == 1){
	...
	console.log(tiempo);
	connection.query('SELECT * FROM comida WHERE tiempo = ? AND tipo = ?',[tiempo, tipo], async (error, results)=>{
		if(error){
            console.log(error);
        }else{
			res.render('gestionTiemposComida',{
...
app.post('/disponibilidad', async(req, res)=>{
	const id = req.body.idComida;
	const disponible = req.body.seleccionDisponible;
	connection.query('SELECT * FROM comida WHERE id = ?', [id], async (error, results)=> {
			...
			connection.query('SELECT * FROM comida', async(error, results)=>{
				if(error){
					...
					res.render('gestionTiemposComida', {
						...
						showConfirmButton: true,
						timer: false,
						ruta: 'gestionTiemposComida'    
					});
				}				
			})
			
		}else{
			connection.query('UPDATE comida SET disponibilidad = ? WHERE id = ?',[disponible, id], async(error, results)=>{
			if(error){
            	console.log(error);
        	}else{
				connection.query('SELECT * FROM comida', async(error, results)=>{
...
//GESTION DE PEDIDOS
app.post('/filtrarPedidos',async(req, res)=>{
	const id = req.body.idPedido;
	if(id !=""){
		connection.query('SELECT * FROM pedidos WHERE id = ?', [id], async (error, results)=> {
			if (results.length == 0 || results[0].id != id){
				connection.query('SELECT * FROM pedidos', async(error, results)=>{
					if(error){
						console.log(error);
					}else{
						res.render('gestionPedidos', {
...
app.post('/modificarPedidos', async (req, res)=>{
	const idu = req.body.idPedido;
	const cedulaN = req.body.cedulaPedido;
    const carneN = req.body.carnePedido;
    const nombreN = req.body.nombrePedido;
    const fecha = req.body.fechaPedido;
	const correoN = req.body.correoPedido;
	const alimento = req.body.alimentosPedido;
	console.log(idu);
	if (idu){
		//se valida si los datos ingresados estan en la base de datos
		connection.query('SELECT * FROM pedidos WHERE id = ?', [idu], async (error, results)=> {
			if (results.length == 0 || results[0].id != idu){
				connection.query('SELECT * FROM pedidos',async(error, results)=>{
					if (error){
						console.log(error);
    			...
    			connection.query('UPDATE pedidos SET cedula = ?, carne = ?, nombreCompleto = ?, fecha = ?, correo = ?, alimentos = ? WHERE id = ?', [cedulaN, carneN, nombreN, fecha, correoN, alimento, idu], async(error, results)=>{
        		if(error){
            		console.log(error);
        		}else{
					res.render('gestionPedidos', {
						...
						correo: correoU,
						alert: true,
						alertTitle: 'Modificación de Pedido',
						alertMessage: 'Cambio Éxitoso!',
						alertIcon: 'success',
						showConfirmButton: false,
						timer: 1500,
	...
	if (id){
	  ...
	  connection.query('SELECT * FROM pedidos WHERE id = ?', [id], async (error, results)=> {
		if (results.length == 0 || results[0].id != id){
		  ...
		  connection.query('DELETE FROM pedidos WHERE id = ?',[id], async(error, results)=>{
			if(error){
			console.log(error);
		  }else{
			connection.query('SELECT * FROM pedidos', async(error, results)=>{
			  ...
			  correo: correoU,
			  alert: true,
			  alertTitle: "Eliminación del Pedido",
			  alertMessage: "Pedido Eliminado con Éxito!",
			  alertIcon:'success',
			  showConfirmButton: false,
			  timer: 1500,
			  ruta: 'gestionPedidos'
...
app.post('/register', async (req, res)=>{
	...
	if (correo.includes("@estudiantec.cr")) { 
		...
		res.render('register', {
			alert: true,
			alertTitle: "Error",
			alertMessage: "El correo no aceptado",
			alertIcon:'error',
			showConfirmButton: true,
			timer: false,
			ruta: 'register'    
